...
function iweb() {
	...
	$getEsencias = db_query("SELECT tiene.nombreobj, objetos.img, tiene.cantidad, encantamientos.id, encantamientos.img FROM tiene JOIN objetos ON (tiene.nombreobj=objetos.nombreobj) JOIN encantamientos ON (tiene.nombreobj=encantamientos.esencia) WHERE tiene.nombrejug='{$jug}' AND tiene.nombreobj LIKE 'Esencia: %'");
	...
	echo '	</select><br>
		...
		';
	//<input style="vertical-align: middle;" type="checkbox" id="checkEsencia" name="esenciaEsp" alt="Esencia de hechizo" title="Añadir Esencia de hechizo" value="esenciaEsp" disabled><label><b id="cantEsencia">x<input id="numeroEsencias" name ="numeroEsencias" type="number" value="1" min="1" max="1" onchange="changePorc(this.value)" alt="Número de esencias" title="Número de esencias" disabled></b> <img id="imgEsencia" src="'.$imgroot.'polest.gif"/> <span id="nombEsencia">Esencia de hechizo</span> <b id="porcEsencia">(+1%)</b> <span style="color:red">[max: 80%]</span></label>
	...
	echo    '];
			...
			if(mysqli_num_rows($getEsencias)){
				for($i=0;$i<mysqli_num_rows($getEsencias);$i++){
					$ess = mysqli_fetch_row($getEsencias);
					echo "'{$ess[0]}#{$ess[1]}#{$ess[2]}#{$ess[3]}#{$ess[4]}'";
					if($i != mysqli_num_rows($getEsencias)-1)
						echo ',';
				}
			}
	echo	'];
			...
			if($spell!=0 && $item!=0){
				...
				if($nSpellDB>0 && $nItem>0){
					...
					if($mc==false && $me==false){
						...
						/*if(isset($_POST['esenciaEsp'])){
							$cantidad = $_POST['numeroEsencias'];
							$prob +=$cantidad;
							$essence = $_POST['essenceName'];
							$essence = explode("#",$essence);
							quita_objeto($jug, $essence[0], $essence[1], $cantidad);
						}*/
						$getSpell = db_query("SELECT id, esencia FROM `encantamientos` WHERE nombreobj='{$spell[0]}'");
						...
						if($rand <= $prob){
							...
							/*$img = db_query("SELECT img FROM objetos WHERE nombreobj='{$spellId[1]}'");
							$img = mysqli_fetch_row($img)[0];
							$esencia = $spellId[1];
							dale_objeto($jug,$esencia);*/
							//$_SESSION['error'] = "¡Algo salió mal y el ritual fracasó! El pergamino se quemó".(isset($_POST['cristal'])||isset($_POST['cristal'])?' y los catalizadores colapsaron.':'.').'<br>Recibiste <img src="'.$imgroot.$img.'.gif"/> <b>x1</b> '.$esencia;
