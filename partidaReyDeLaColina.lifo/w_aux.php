...
function get_hora($time)
{
  return date('H', l_getdate($time));
}
function get_minutos($time)
{
  return date('i', l_getdate($time));
}
...
{
  global $time, $confvidabase, $confvidanivel, $confascension;
  ...
  if ($sombra != 1) {
    $ataqasc = floor($ataquebase * (($confascension / 100) * (getAscensiones($nombre))));
    $protasc = floor($proteccionbase * (($confascension / 100) * (getAscensiones($nombre))));
    $vidaasc = floor($vidabase * (($confascension / 100) * (getAscensiones($nombre))));
    $juga['ataqprotinvent'] += $ataqasc + $protasc;
  } else {
    $ataqasc = 0;
    $protasc = 0;
    $vidaasc = 0;
  ...
  $juga['ataq'] = $juga['ataq'] + $juga['ataqmascota'] + $encantt[0] + $ataqasc;
  $juga['prot'] = $juga['prot'] + $juga['protmascota'] + $encantt[1] + $protasc;
  $juga['vida'] = $juga['vida'] + $juga['vidamascota'] + $encantt[2] + $vidaasc;
...
function combateFinish($jug)
{
  ...
  switch ($random) {
    ...
    case 10:
      ...
      break;
    case 11:
      $str = 'se comió a ' . $jug . ' y se merendó a su mascota';
      break;
    default:
      $str = 'ha ganado el combate';
      break;
...
{
  global $confobj, $confnivel, $time, $imgroot, $confencobjbase,$confencobjeven;
  $cambiopuntos = 1;
  ...
  while ($cambiopuntos) {
    ...
    if ($numrows > 0) {
      ...
      if ($mens == "Has encontrado:<br/><br/>")
        $mens = '';
      if ($nivel < 100) {
        while ($puntos >= expsignivel($nivel, $ultimossubio) && $nivel < 100) {
          $ultimossubio = $ultimossubio + $nivel * $confnivel;
          $nivel++;
          $mens .= "¡Enhorabuena, has subido hasta el nivel <b>{$nivel}</b>!<br/>";
        }

        if ($nivel == 100) {
          $ultimossubio = 0;
          $mens .= "<br/><br/>¡Enhorabuena, has subido a <b>nivel máximo</b>! ¡Eres una bestia!<br/><br/>";
        }
      }
...
function getImgEnchanted($img, $tipo, $enchantid)
{
  ...
  if ($enchantlvl >= 80) {
    return "<img style=\"background:url({$imgroot}{$img}.gif)\" src=\"{$imgroot}\/enchant/enchanted5.gif\" alt=\"{$tipo} encantado\"/>";
  } else if ($enchantlvl >= 60) {
    return "<img style=\"background:url({$imgroot}{$img}.gif)\" src=\"{$imgroot}\/enchant/enchanted4.gif\" alt=\"{$tipo} encantado\"/>";
  } else if ($enchantlvl >= 40) {
    return "<img style=\"background:url({$imgroot}{$img}.gif)\" src=\"{$imgroot}\/enchant/enchanted3.gif\" alt=\"{$tipo} encantado\"/>";
  } else if ($enchantlvl >= 20) {
    return "<img style=\"background:url({$imgroot}{$img}.gif)\" src=\"{$imgroot}\/enchant/enchanted2.gif\" alt=\"{$tipo} encantado\"/>";
  } else {
    return "<img style=\"background:url({$imgroot}{$img}.gif)\" src=\"{$imgroot}\/enchant/enchanted.gif\" alt=\"{$tipo} encantado\"/>";
  }
...
function puedeascender($jug)
{
  ...
  $proximaAscension = 10 + $ascensiones * 10;
  if ($ascensiones < 5 && $nivel >= $proximaAscension)
    return true;
  return false;
}
...
function mostrarCampana($campana)
{
  global $imgroot, $tag;
  $html = '';
  $html .= '<tr><td align="center" style="border: 3px ridge grey;border-radius: 10px; width: 180px; height: 100px; background-image: url(' . $imgroot . 'woodbackground.png)">
    <form action="campanasreales.php" method="post">
    <input type="hidden" name="tag" value="' . $tag . '"/>
    ';
  $text = '';
  switch ($campana) {
    case 1:
      $html .= '<img src="' . $imgroot . 'campanahonor.gif" alt="Campana del honor">';
      $text = 'Tocar Campana del honor';
      break;
    case 2:
      $html .= '<img src="' . $imgroot . 'campanasangre.gif" alt="Campana de sangre">';
      $text = 'Tocar Campana de sangre';
      break;
    case 3:
      $html .= '<img src="' . $imgroot . 'campanadestreza.gif" alt="Campana de la destreza">';
      $text = 'Tocar Campana de la destreza';
      break;
    case 4:
      $html .= '<img src="' . $imgroot . 'campanaamistad.gif" alt="Campana de la amistad">';
      $text = 'Tocar Campana de la amistad';
      break;
    case 5:
      $html .= '<img src="' . $imgroot . 'campanasaber.gif" alt="Campana del saber">';
      $text = 'Tocar Campana del saber';
      break;
    case 6:
      $html .= '<img src="' . $imgroot . 'campanamisterio.gif" alt="Campana del misterio">';
      $text = 'Tocar Campana del misterio';
      break;
  }
  $html .= '<input type="hidden" name="campana" value="' . $campana . '"/>
  <br/><input class="btn_Enchant" style="background-image: none; cursor: pointer;" type="submit" name="tocarcampana" value="' . $text . '"/><br/>
  ';
  $html .= '</form></td></tr>';
  return $html;
}
...
function getReducDanio($nivel, $armadura)
{
...
function combateContraEnano()
{
  global $imgroot, $jug, $time, $confnoatacarhastapremium, $confnoatacarhasta;
  // DATA: id, nombre, img, nivel, oromin, oromax, expmin, expmax, dmg, hp, crit, evasion
  $enano = mysqli_fetch_assoc(db_query("SELECT * FROM `monstruos` WHERE `nombre`='Defensor Myrrodir'"));
  $enano['vida'] = $enano['hp'];
  $enano['ataq'] = $enano['dmg'];
  $enano['prot'] = 0;
  $jugador = combateinfo($jug, 0);
  $textoCombate = "
  Mientras explorabas los interiores del volcán, tu amuleto empieza a vibrar.<br>
  Notas que al girarte y caminar en una dirección la vibración aumenta.<br>
  Continúas hasta que encuentras un portón sellado con antiguas inscripciones <br>
  enanas de otra era. Tu amuleto emite un sonido agudo mientras brilla.<br>
  Acto seguido, el portón ante tí se abre, revelando un antiguo custodio enano arrodillado que <br>
  protege un cofre. El custodio, al verte, se pone en posición y se lanza a hacía ti.<br>
  <br>
  ¡Prepárate! ¡Comienza el combate!<br><br>";
  ...
  $textoCombate .= "<img src=\"{$imgroot}{$enano['img']}.gif\"/> <strong>{$enano['nombre']}</strong> (nivel {$enano['nivel']})<br>";

  $vencedor = true;
  $aniquilado = false;

  if (mysqli_fetch_row(db_query("SELECT COUNT(*) FROM tiene WHERE nombrejug='{$jug}' AND usado=1 AND nombreobj LIKE '%fuego mágico%'"))[0] != 6) {
    $aniquilado = true;
  }
  $textoCombate .= "<b>[COMIENZO DEL COMBATE]</b><br>";
  $textoCombate .= vidaActual($jugador, $enano);
  if ($aniquilado == false) {
    $turno = rand(0, 1);
    if ($turno == 1) $textoCombate .= "¡<b>{$jugador['nombre']}</b> comienza golpeando!<br>";
    else $textoCombate .= "¡<b>{$enano['nombre']}</b> comienza golpeando!<br>";
    while ($jugador['vida'] > 0 && $enano['vida'] > 0) {
      $textoCombate .= gestionaGolpe($jugador, $enano, $turno);
      $textoCombate .= vidaActual($jugador, $enano);
      if ($turno == 1) $turno = 0;
      else $turno = 1;
    }
    if ($jugador['vida'] == 0) $vencedor = false;
  }

  if ($aniquilado == true) {
    $textoCombate .= "¡<b>{$enano['nombre']}</b> comienza golpeando!<br><br>
    <i>El enano te mira fíjamente, se ríe, y sin decir nada te asesta un tajo certero.</i><br>
    [Myrrodyr]: <strong>NO ERES DIGNO.</strong><br><br>";
    $textoCombate .= "¡<b>{$enano['nombre']}</b> te asesta un golpe mortal de {$jugador['vida']} puntos!<br><br>
    [Myrrodyr]: <strong>VETE, MORTAL.</strong><br><br>";

    $textoCombate .= "¡{$jugador['nombre']} pierde el encuentro!<br>";
    $exp = round(rand($enano['expmin'], $enano['expmax']) * (1 / 3));
    $textoCombate .= "¡{$jugador['nombre']} pierde {$exp} puntos de experiencia!<br>";
    $exp = -$exp;
    $oro = round(rand($enano['oromin'], $enano['oromax']) * (1 / 3));
    ...
    if ($vencedor == false) {
      ...
      $exp = rand($enano['expmin'], $enano['expmax']);
      $textoCombate .= "¡{$jugador['nombre']} gana {$exp} puntos de experiencia!<br>";
      $oro = rand($enano['oromin'], $enano['oromax']);
      $textoCombate .= "¡{$jugador['nombre']} gana {$oro} monedas de oro!<br><br>";

      $textoCombate .= "Tras derrotar al custodio, se arrodilla ante tí.<br>
      [Myrrodyr]: <strong>POR FIN... PODRÉ... DESCANSAR...<br>PADRE, ¿ERES TÚ?...</strong><br>
      <i>Myrrodir cae rendido al suelo y se desvanece.</i><br><br>
      Te adelantas por fin, y recoges el cofre huyendo del lugar.<br><br>";

      $textoCombate .= "¡Has recibido <img src=\"{$imgroot}cenano.gif\"/> <b>x1</b> Cofre enano!";
      dale_objeto($jug,"Cofre enano",1);
      $jugador['vencedor'] += 1;
    }
  }
  $jugador['puntossuma'] += $exp;
  $jugador['oro'] += $oro;
  if ($jugador['premiumhasta'] >= $time)
    $jugador['noatacarhasta'] = $time + $confnoatacarhastapremium;
  else
    $jugador['noatacarhasta'] = $time + $confnoatacarhasta;
  db_query("UPDATE jugadores SET puntossuma={$jugador['puntossuma']},oro={$jugador['oro']},noatacarhasta={$jugador['noatacarhasta']},combates={$jugador['combates']},vencedor={$jugador['vencedor']},vencido={$jugador['vencido']} WHERE nombrejug='{$jug}'");
  db_query("INSERT INTO mensajes (nombrejug,remitente,hora,mensaje,tipo,visto) VALUES ('{$jug}','@',{$time},'{$textoCombate}','SECRETO',0)");
  sumapuntos($jug);
}

function gestionaGolpe(&$jugador, &$monstruo, $turno)
{
  $textoCombate = '';
  if ($turno == 1) { //GOLPE DEL JUGADOR
    $danio = rand($jugador['ataq'] * 0.9, $jugador['ataq'] * 1.1);
    if (rand(1, 100) <= 10) {
      $danio *= 2;
      $textoCombate .= "<b>¡{$jugador['nombre']}</b> asesta un golpe <b>CRÍTICO</b> a <b>{$monstruo['nombre']}</b> de <b>{$danio}</b> puntos de vida!<br>";
    } else {
      $textoCombate .= "<b>{$jugador['nombre']}</b> asesta un golpe a <b>{$monstruo['nombre']}</b> de <b>{$danio}</b> puntos de vida.<br>";
    }
    $monstruo['vida'] -= $danio;
    if ($monstruo['vida'] < 0) $monstruo['vida'] = 0;
  } else { //GOLPE DEL ENANO
    $danio = rand($monstruo['ataq'] * 0.5, $monstruo['ataq'] * 1);
    $danio = $danio - floor($danio * (getReducDanio($jugador['nivel'], $jugador['prot']) / 100));
    if (rand(1, 100) <= 10) {
      $danio = 0;
      $textoCombate .= "<b>{$monstruo['nombre']}</b> asesta un golpe, ¡pero <b>{$jugador['nombre']}</b> lo evade!<br>";
    } else {
      $textoCombate .= "<b>{$monstruo['nombre']}</b> asesta un golpe a <b>{$jugador['nombre']}</b> de <b>{$danio}</b> puntos de vida.<br>";
    }
    $jugador['vida'] -= $danio;
...
function vidaActual($jugador, $monstruo)
{
  return "Puntos de vida [<b>{$jugador['nombre']}</b>: {$jugador['vida']}] - [<b>{$monstruo['nombre']}</b>: {$monstruo['vida']}]<br><br>";
}

function conoceReceta($jug, $receta)
{
  $conoce = false;
  $it = db_query("SELECT * FROM conoce WHERE nombrejug='{$jug}' AND resultado='{$receta}'");
  if (mysqli_num_rows($it))
    $conoce = true;
  return $conoce;
}

function tieneOtraEra($player)
{
	$tiene = false;
	if(tieneItem($player, 'Cofre enano') || tieneItem($player, 'Receta: Amuleto de otra era') || conoceReceta($player, 'Amuleto de otra era')){
		$tiene = true;
	}
	return $tiene;
}