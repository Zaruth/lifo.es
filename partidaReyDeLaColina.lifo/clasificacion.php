...
function iweb() {
  ...
  $retval = db_query("SELECT clan,SUM(puntos) a,0,SUM(vencedor) v,SUM(combates),COUNT(*) FROM {$conftp}jugadores WHERE {$conftp}jugadores.clan<>'(ninguno)' && clan != 'Administracion' GROUP BY clan ORDER BY a DESC,v DESC,clan DESC");
  $retvall = db_query("SELECT clan,nombrejug FROM {$conftp}clandata WHERE lider=1 && clan != 'Administracion'");
  db_unlock();
