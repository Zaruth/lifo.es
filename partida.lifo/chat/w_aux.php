...
function mail_recu($user, $email)
{
  ...
  // título
  $título = ' - Email de recuperación de contraseña.';
  ...
  $cabeceras .= 'From: Recordatorio de Contraseña ' . "\r\n";
...
function combate($atacante, $atacado, $aleat)
{
  ...
  db_query("INSERT INTO {$conftp}mensajes (nombrejug,remitente,hora,mensaje,tipo) VALUES ('{$atacante}','@',$time,'{$mensaje}','Combate')");
  if ($atacado != "Xilok" && $atacante != $atacado) {
    db_query("INSERT INTO {$conftp}mensajes (nombrejug,remitente,hora,mensaje,tipo) VALUES ('{$atacado}','@',$time,'{$mensaje}','Combate')");
  }
  if ($atacado == "Xilok") {
    db_query("UPDATE {$conftp}jugadores SET noatacarhasta={$tacante['noatacarhasta']},protegidohasta=0 WHERE nombrejug='{$atacante}'");
  } else {
    if ($atacante == $atacado) {
      db_query("UPDATE {$conftp}jugadores SET puntossuma={$tacante['puntossuma']},noatacarhasta={$tacante['noatacarhasta']},protegidohasta=0,combates={$tacante['combates']},vencedor={$tacante['vencedor']},vencido={$tacante['vencido']} WHERE nombrejug='{$atacante}'");
    } else {
  ...
  if ($atacado == "Xilok" || $atacante == $atacado) {
    if ($atacado == "Xilok") {
      db_query("UPDATE {$conftp}jugadores SET oro=0,puntossuma=0,protegidohasta=0,combates=0,vencedor=0,vencido=0 WHERE nombrejug='{$atacado}'");
    }
  } else {
...
function sumapuntos($jug)
{
  ...
  while ($cambiopuntos) {
    ...
    $retval = db_query("SELECT puntos,nivel,puntosnivel,puntosobjeto,puntossuma FROM {$conftp}jugadores WHERE puntossuma<>0 AND nombrejug='{$jug}' ORDER BY login DESC LIMIT 0,20");
    $numrows = mysqli_num_rows($retval);
    if ($numrows > 0) {
      $mens = '';
      $ret = mysqli_fetch_row($retval);
...
{
  global $conftp, $time;
  $retval = db_query("SELECT * FROM {$conftp}sorteos WHERE fechalimite >= {$time} AND ganador = ''");
...
{
  global $conftp;
  db_query("UPDATE {$conftp}jugadores SET oro=oro+{$canti} WHERE nombrejug='{$jug}'");
}
...
function postEmail($toEmail, $filePath){
	db_query("INSERT INTO lifoes_especial.emails(`email`, `emailToSend`) VALUES ('{$toEmail}', '{$filePath}')");
}
