...
";
  ...
  echo 'Dar logro.<br/>';
  echo '<form action="admin.php" method="post">Jugador: <input type="text" name="jugador"/> Id logro: <input type="text" name="idlogro"/> <input type="submit" name="darlogro"/></form><br/><br/>';
...
function procesaform()
{
  ...
  if (isset($_REQUEST['darlogro'])) {
    $jugador = check_username($_REQUEST['jugador']);
    $idlogro = $_REQUEST['idlogro'];
	include_once 'auxiliar/f_logros.php';
	dar_logro ($jugador,$idlogro);
    $_SESSION['mensaje'] = "Logro {$idlogro} otorgado a {$jugador}.";
    header('Location: admin.php');
    return 1;
  }
