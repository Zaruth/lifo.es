...
function iweb()
{
  ...
  echo "<b>Repite E-mail: *</b><br/>";
  echo "<input type=\"text\" name=\"email2\" size=\"25\" maxlength=\"80\" value=\"{$lastemail}\" onpaste=\"return false;\" ondrop=\"return false;\" autocomplete=\"off\"/><br/><br/>";
  ...
  echo "<input type=\"submit\" name=\"registra\" value=\"Registrarse\" onclick=\"c('Revisa bien tu correo, se te enviará un mensaje para verificar tu cuenta.')\"/> <b>* obligatorio</b><br/>";
...
function procesaform()
{
  ...
  if (isset($_REQUEST['registra'])) {
    ...
    if ($ident)
      ...
      if ((isset($_REQUEST['username'])) && (isset($_REQUEST['email'])) && (isset($_REQUEST['email2'])) && (isset($_REQUEST['newpass'])) && (isset($_REQUEST['newpassc']))) {
        $username = check_username($_REQUEST['username']);
        $email = check_email($_REQUEST['email']);
        $email2 = check_email($_REQUEST['email2']);
        ...
        elseif (!$username) {
          ...
          if ($a) {
            ...
            if ($email != $email2) {
              $_SESSION['error'] = 'Las direcciones de email no coinciden.';
            } else {
              if ((!$newpass) || (!$newpassc))
                ...
                if ($newpass != $newpassc)
                  ...
                  if (antiflood('registro', $ip, 2, 3600)) {
                    ...
                    db_query("INSERT INTO jugadores (nombrejug,password,email,verificado,emailreg,creado,ipcreado,iplogin,padre) VALUES ('{$username}','{$pwdhash}','{$email}',0,'{$email}',{$time},'{$ip}','(no)','{$padre}')");
