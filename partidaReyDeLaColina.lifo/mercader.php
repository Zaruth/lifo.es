...
function iweb() {
  ...
  if ($of) {
    ...
    db_lock("objetos READ,ofertasesp READ,tiene READ");
    $retval3 = db_query("SELECT ofertasesp.nombreobj,tipo,img,ataq,prot,valor,niveluso,usos,unico,preciojoyas FROM objetos,ofertasesp WHERE objetos.nombreobj=ofertasesp.nombreobj ORDER BY tipo ASC");
    ...
    for ($i = 0;$i < $numrows;$i++) {
      ...
      if ($ret[7] <= 1) {
        if ($ret[7])
         ...
         $usos = '';
         echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio: <img src=\"{$imgroot}rprem.gif\" alt=\"Premium\"/>x{$ret[9]}.</b> {$ret[0]} (Ataque: {$ret[3]} / Protección: {$ret[4]} / Nivel: {$ret[6]}{$usos}{$valorenoro})<br/></div>";
      }
      else if ($ret[7] == 2) {
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio: <img src=\"{$imgroot}rprem.gif\" alt=\"Premium\"/>x{$ret[9]}.</b> {$ret[0]} (Energía: +{$ret[4]} / UN SOLO USO{$valorenoro})<br/></div>";      
      }
      else if ($ret[7] == 3) {
        $aprendido = '';
        for ($ba = 0;$ba < $nrb;$ba++) {
        ...
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio: <img src=\"{$imgroot}rprem.gif\" alt=\"Premium\"/>x{$ret[9]}.</b> {$ret[0]} (Energía: -{$ret[3]} / UN SOLO USO{$aprendido}{$valorenoro})<br/></div>";
      }
      else if ($ret[7] == 4) {
        if ($valorenoro!='')
          $valorenoro='('.substr($valorenoro,3).')';
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio: <img src=\"{$imgroot}rprem.gif\" alt=\"Premium\"/>x{$ret[9]}.</b> {$ret[0]} {$valorenoro}<br/></div>";      
      }
      else if ($ret[7] == 5) {
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio: <img src=\"{$imgroot}rprem.gif\" alt=\"Premium\"/>x{$ret[9]}.</b> {$ret[0]} (consumir para obtener 1 semana premium{$valorenoro})<br/></div>";      
      }
      else if ($ret[7] == 6) {
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio: <img src=\"{$imgroot}rprem.gif\" alt=\"Premium\"/>x{$ret[9]}.</b> {$ret[0]} (MASCOTA{$valorenoro})<br/></div>";      
      }
      else if ($ret[7] == 7) {
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio: <img src=\"{$imgroot}rprem.gif\" alt=\"Premium\"/>x{$ret[9]}.</b> {$ret[0]} (Energía: -{$ret[3]} / Nivel: {$ret[6]} / MAPA{$valorenoro})<br/></div>";      
      }
      else if ($ret[7] == 8) {
        if ($valorenoro!='')
          $valorenoro='('.substr($valorenoro,3);
  ...
  /*$retval = db_query("SELECT * FROM lasttime");
  ...
  if ($veces) {
    ...
    if (!$ret[0]) {
      $veces = 0;
      db_query("UPDATE lasttime SET lasttime={$time}");
    }
    else {
      $veces = floor(($time-$ret[0])/$confmercencuentra);
      $vecesall = $veces;
      ...
      if ($veces>0) {
        $retval2 = db_query("SELECT MAX(nivel) FROM jugadores");
        ...
        $retval2 = db_query("SELECT nombreobj,posibilidad,img,tipo,ataq,prot,puntosencontrar,nivelcomprar FROM objetos WHERE nivelencontrar<={$maxniv} AND nivelencontrar>0 ORDER BY posibilidad DESC");
        ...
        if ($maxpos == 0)
          $veces = 0;
        mysqli_free_result($retval2);
        db_query("UPDATE lasttime SET lasttime=lasttime+{$confmercencuentra}*{$vecesall}");
      }
      for ($x = 0;$x < $veces;$x++) {
  ...
  $retval2 = db_query("SELECT tiene.nombreobj,cantidad,tipo,img,ataq,prot,valor,usado,niveluso,usos FROM tiene,objetos WHERE nombrejug='{$jug}' AND tiene.nombreobj=objetos.nombreobj AND tiene.encantamiento=0 ORDER BY tipo ASC,ataq+prot DESC,posibilidad ASC");
  $retvalpp = db_query("SELECT cantidad FROM tiene WHERE nombrejug='{$jug}' AND nombreobj='Joya premium'");
  ...
  if ($cat) {
    ...
    db_lock("objetos READ,ofertas READ");
    ...
    $retval3 = db_query("SELECT nombreobj,tipo,img,ataq,prot,valor,niveluso,usos,posibilidad FROM objetos WHERE ((niveluso<={$us['nivel']}) OR (valor<={$us['oro']})) AND ((nivelcomprar>0 AND nivelcomprar<={$us['nivel']}) OR nombreobj IN {$query}) AND tipo='{$cat}' ORDER BY ataq+prot DESC,posibilidad ASC LIMIT {$ini},1000");
    ...
    for ($i = 0;$i < $numrows;$i++) {
      ...
      if ($ret[7] <= 1) {
        ...
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio compra: {$ret[5]}.</b> {$ret[0]} (Energía: -{$ret[3]} / Nivel: {$ret[6]} / MAPA)<br/></div>";      
      }
      else if ($ret[7] == 8) {
        echo "<img src=\"{$imgroot}{$ret[2]}.gif\" alt=\"{$ret[1]}\"/> <b>Precio compra: {$ret[5]}.</b> {$ret[0]}<br/></div>";      
      }
    ...
    $query .= '\'0_$\')';
    mysqli_free_result($retvaloff);
    $retvaln = db_query("SELECT DISTINCT tipo,COUNT(*) FROM objetos WHERE ((niveluso<={$us['nivel']}) OR (valor<={$us['oro']})) AND ((nivelcomprar>0 AND nivelcomprar<={$us['nivel']}) OR nombreobj IN $query) GROUP by tipo");
    ...
    for ($i = 0;$i < $numrows;$i++) {
      ...
      if ($ret[9] <= 1) {
        ...
        echo "<img src=\"{$imgroot}{$ret[3]}.gif\" alt=\"{$ret[2]}\"/><b>x{$ret[1]} Precio venta: {$venta}.</b> {$ret[0]} (Energía: -{$ret[4]} / Nivel: {$ret[8]} / MAPA)<br/></div>";
      }
      else if ($ret[9] == 8) {
        echo "<img src=\"{$imgroot}{$ret[3]}.gif\" alt=\"{$ret[2]}\"/><b>x{$ret[1]} Precio venta: {$venta}.</b> {$ret[0]}<br/></div>";
      }
...
function procesaform() {
  ...
  if (isset($_REQUEST['comprar'])) {
    if ((isset($_REQUEST['tag'])) && (isset($_REQUEST['idob'])) && (isset($_REQUEST['cantidad']))) {
      ...
      $cantidad = floor($_REQUEST['cantidad']);
      if (($tag == $gtag) && (preg_match('/^[a-z]+$/',$idob)) && (is_numeric($cantidad)) && ($cantidad > 0)) {
        db_lock("jugadores WRITE,tiene WRITE,objetos READ,ofertas WRITE");
