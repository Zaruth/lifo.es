...
function iweb()
{
  ...
  if (isset($c)) {
    ...
    $retvalcat = db_query(
      "SELECT DISTINCT(obj.tipo),obj.usos,(
    								SELECT IF(ti2.encantamiento!=0,1,ti2.usado)
    									FROM tiene AS ti2 JOIN objetos AS obj2 ON (ti2.nombreobj=obj2.nombreobj) 
    										WHERE ti2.nombrejug='{$jug}' AND obj2.tipo=obj.tipo AND obj2.usos='0' AND obj2.niveluso<={$us['nivel']}
    											ORDER BY obj2.ataq+obj2.prot DESC, obj2.niveluso DESC, ti2.encantamiento DESC, obj2.nombreobj ASC
    												LIMIT 1
									) AS usado
        FROM tiene AS ti JOIN objetos AS obj ON (ti.nombreobj=obj.nombreobj) 
            WHERE ti.nombrejug='{$jug}' AND obj.tipo != 'Mapa'
  ...
  if ($Jugador->sets != null) {

    echo "<div class=\"bonificacion\"><table><tr><td>
    <table style=\"white-space: nowrap; width: 100%;\"><tbody><tr><td>";

    echo "<table><tr><td style=\"color: #ffd100;\">Conjunto</td><td style=\"color: #ffd100\">Bonificación</td></tr>";
    for ($i = 0; $i < count($Jugador->sets); $i++) {
      $itemSet = $Jugador->sets[$i];
      echo "<tr><td style=\"text-align:left; padding-right: 5px; color: " . ($itemSet->numpiezasjug < $itemSet->minpiezas ? '#9d9d9d' : $itemSet->color) . ";\">{$itemSet->nombreset} ({$itemSet->numpiezasjug}/{$itemSet->maxpiezas})</td>";
      if ($itemSet->numpiezasjug < $itemSet->minpiezas) {
        $num = $itemSet->minpiezas - $itemSet->numpiezasjug;
        echo "<td style=\"color: #9d9d9d;\">[Te faltan {$num} pieza" . ($num > 1 ? 's' : '') . " para el bonus]</td>";
      } else
        echo "<td style=\"color: #1eff00;\">" . $itemSet->getBonusString() . "</td>";
      echo "</tr>";
    }
    echo "</table>";
            ...
            if (!isset($item['enniveluso']) || (isset($item['enniveluso']) && $item['niveluso'] >= $item['enniveluso'])) {
              if ($item['niveluso'] > $us['nivel']) {
                ...
                if ($item['encantamiento'] != 0) {
                  $encnom = db_query("SELECT con.nombreobj
                                      FROM encantamientos as enc JOIN contiene as con ON (enc.nombreobj=con.resultado) 
                                      WHERE enc.id={$item['encantamiento']}");
                  $libro = mysqli_fetch_row($encnom)[0];
                  $retv5 = db_query("SELECT con.nombreobj, COUNT(con.nombreobj) as numenc
                                    FROM tiene as ti JOIN encantamientos as enc ON (ti.encantamiento=enc.id) JOIN contiene as con ON (enc.nombreobj=con.resultado)
                                    WHERE ti.nombrejug='{$jug}' AND ti.usado=1 AND ti.encantamiento!=0
                                    GROUP BY con.nombreobj");
                  if (mysqli_num_rows($retv5)) {
                    for ($j = 0; $j < mysqli_num_rows($retv5); $j++) {
                      $row = mysqli_fetch_row($retv5);
                      if ($libro == $row[0]) {
                        $encyaequip = true;
                        break;
                      }
...
{
  global $_REQUEST, $tag, $jug, $confnocomerhastapremium, $confnocomerhasta, $confnoexplorarhasta, $confnoexplorarhastapremium, $time, $us, $imgroot;
      ...
      if ($tag == $gtag) {
        ...
        if (mysqli_num_rows($retval) == 1) {
          ...
          if (mysqli_num_rows($retvol) == 1) {
            ...
            db_query("UPDATE jugadores SET premiumhasta={$time}+86400*3 WHERE nombrejug='{$jug}' AND premiumhasta<{$time}");
        ...
        if (mysqli_num_rows($retv2)) {
          ...
          if (mysqli_num_rows($retv3)) {
            ...
            if ($item1[2] != 0) {
              $encnom = db_query("SELECT con.nombreobj
                                  FROM encantamientos as enc JOIN contiene as con ON (enc.nombreobj=con.resultado) 
                                  WHERE enc.id={$item1[2]}");
              $libro = mysqli_fetch_row($encnom)[0];
              $retv5 = db_query("SELECT con.nombreobj, COUNT(con.nombreobj) as numenc
                                FROM tiene as ti JOIN encantamientos as enc ON (ti.encantamiento=enc.id) JOIN contiene as con ON (enc.nombreobj=con.resultado)
                                WHERE nombrejug='{$jug}' AND usado=1 AND encantamiento!=0
                                GROUP BY con.nombreobj");
              if (mysqli_num_rows($retv5)) {
                for ($i = 0; $i < mysqli_num_rows($retv5); $i++) {
                  $row = mysqli_fetch_row($retv5);
                  if ($libro == $row[0]) {
                    $yatieneenc = true;
                    break;
            ...
            if ($yatieneenc == false) {
              ...
              $_SESSION['error'] = 'Ya estás usando otro objeto con un encantamiento del mismo libro.<br>No puedes usar dos objetos con un encantamiento de un mismo libro.';
          } else
