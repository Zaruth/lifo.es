...
if (isset($_SESSION['identificado'])) {
  if ((isset($_SESSION['jugador'])) && (isset($_SESSION['pwd']))) {
    ...
    db_lock("{$conftp}jugadores READ");
    $retval = db_query("SELECT baneadohasta,admin,premiumhasta,moderador,ultimoforo,nivel,puntos,trabajado,clan,
    email,trabajando,fintrabajo,energia,oro,protegidohasta,noatacarhasta,creado,puntosnivel,hijos,nietos,padre,
    combates,vencedor,vencido,insignia,visitashijos,visitasnietos,nocomerhasta,noexplorarhasta,enchufado,enerext,
    verificado,recibemail,iplogin,noencuentrohasta FROM {$conftp}jugadores WHERE nombrejug='{$_SESSION['jugador']}' AND password='{$_SESSION['pwd']}'");
    db_unlock();
    $dentro = 0;
    if (mysqli_num_rows($retval)) {
      $dentro = 1;
