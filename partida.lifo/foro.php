...
function iweb()
{
	...
	if (isset($responderm)) {
		...
		if ($_SESSION['silenciado']) {
			...
			$tabla .= "<br/>
					<input type=\"submit\" name=\"nuevarespuesta2\" onclick=\"return c('Si escribe en el foro público insultos o palabras malsonantes, o amenazas fuera del sentido del juego, su cuenta podrá ser cerrada.')\" value=\"Responder\"/>
					</td>
				</tr>
				</table>";
			if ($us['admin'] == 1 || $us['moderador'] == 1) {
		...
		if ($id) {
			...
			if ($_SESSION['silenciado']) {
				...
				if ($us['admin'] == 1 || $us['moderador'] == 1)
					$tabla .= "<input type=\"checkbox\" name=\"html\" value=\"si\"> Texto Html";
				$tabla .= "<br/><input type=\"submit\" name=\"nuevarespuesta\" onclick=\"return c('Si escribe en el foro público insultos o palabras malsonantes, o amenazas fuera del sentido del juego, su cuenta podrá ser cerrada.')\" value=\"Nueva respuesta\"/></td></tr></table>";
				if ($us['admin'] == 1 || $us['moderador'] == 1) {
					echo $tabla;
				} else {
					if ($cerrada == 0) {
			...
			if ($_SESSION['silenciado']) {
				...
				if ($us['admin'] == 1 || $us['moderador'] == 1)
					$tabla .= "<input type=\"checkbox\" name=\"html\" value=\"si\"> Texto Html";
				$tabla .= "<br/><input type=\"submit\" name=\"nuevahebra\" onclick=\"return c('Si escribe en el foro público insultos o palabras malsonantes, o amenazas fuera del sentido del juego, su cuenta podrá ser cerrada.')\" value=\"Nueva hebra\"/></td></tr></table>";
				if ($us['admin'] == 1 || $us['moderador'] == 1) {
					echo $tabla;
				} else {
					if ($us['nivel'] >= $confnivelforo) {
