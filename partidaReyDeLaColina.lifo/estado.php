...
function iweb()
{
  ...
  db_lock("tienemascotas READ,mascotas READ");
  ...
  $proten = $encantt[1];
  $vidaen = $encantt[2];
  $ataqasc = floor($ataqb * (($confascension / 100) * (getAscensiones($jug))));
  $protasc = floor($protb * (($confascension / 100) * (getAscensiones($jug))));
  $vidaasc = floor($vidab * (($confascension / 100) * (getAscensiones($jug))));
  ...
  if ($ascJug < 5 && 
     ($ascJug >= 2 && $nivelJug < 76)) { // Semana del 14 al 21
  /*
  if ($ascJug < 5 && 
      (($ascJug >= 2 && $nivelJug < 76) ||
      ($ascJug >= 1 && $nivelJug < 76)) { // Semana del 21 al 28
  */
  /*
  if ($ascJug < 5 && 
      (($ascJug >= 2 && $nivelJug < 76) ||
      ($ascJug >= 1 && $nivelJug < 76) ||
      ($ascJug >= 2 && $nivelJug >= 76) ||
      ($ascJug >= 0 && $nivelJug < 76 && $nivelJug >= 10)) { // Semana del 28 al 5
  */
  /*
  if ($ascJug < 5 && 
      (($ascJug >= 2 && $nivelJug < 76) ||
      ($ascJug >= 1 && $nivelJug < 76) ||
      ($ascJug >= 2 && $nivelJug >= 76) ||
      ($ascJug >= 0 && $nivelJug < 76 && $nivelJug >= 10) ||
      ($ascJug >= 1 && $nivelJug >= 76)) { // Semana del 5 al 12
  */
  //if ($ascJug < 5) { // Semana del 19 en adelante

    $proxAscension = "Nivel necesario para próxima ascensión: <b>" . (10 + getAscensiones($jug) * 10) . "</b> <i>-No activo para usuarios con 1 o más ascensiones.-</i>";
    //if ($puedeAsc) {
      $inputAscension = "<tr><td colspan=\"2\">
      <form action=\"estado.php\" method=\"post\">
      <input type=\"hidden\" name=\"tag\" value=\"{$tag}\"/>
      <input type=\"submit\" name=\"ascender\" value=\"Ascender\" alt=\"Ascender\" title=\"Ascender\">
      </form></td></tr>";
      //onclick=\"c('Tu nivel volverá a 1')\"
    //}
  } else {
    if($ascJug == 5){
  ...
  $porcen = getReducDanio($us['nivel'],$prot);
...
<tr><td><b>Ataque (ascensión):</b></td><td>{$ataqasc}</td></tr>
...
<tr><td><b>Protección (ascensión):</b></td><td>{$protasc}</td></tr>
...
<tr><td><b>Porcentaje de reducción de daño:</b></td><td>{$porcen}%</td></tr>
<tr><td colspan=\"2\"><hr align=\"left\" width=\"265px\"></tr>
<tr><td><b>Vida (nivel):</b></td><td>{$vidab}</td></tr>
<tr><td><b>Vida (ascensión):</b></td><td>{$vidaasc}</td></tr>
...
<tr><td colspan=\"2\"><b>[Bonus stats] Daño: +" . ($ascen * $confascension) . "% || Protección: +" . ($ascen * $confascension) . "% ||  Vida: +" . ($ascen * $confascension) . "%</b></td></tr>
<tr><td colspan=\"2\"><i>-Un gnomo mágico te impide ascender.-</i></td></tr>
</table><br/><br/><br/>
";
